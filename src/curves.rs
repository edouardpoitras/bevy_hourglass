...
/// Generate outline points with mounded fill line for bottom bulb
fn generate_outline_with_mounded_fill_line(
    ...
    if bottom_mound_factor == 0.0 {
        ...
        return generate_outline_with_fill_line(hourglass_outline, base_fill_line, bulb, center_y);
    }
    ...
    for i in 0..hourglass_outline.len() {
        ...
        // Check if current point should be included based on bulb and mounded fill level
        ...
        let next_included = match bulb {
            SandBulb::Top => next_point[1] >= center_y && next_point[1] <= next_mounded_fill,
            SandBulb::Bottom => next_point[1] <= center_y && next_point[1] <= next_mounded_fill,
        };
        ...
        if current_included != next_included {
            ...
            for j in 1..samples {
                ...
                let sample_included = match bulb {
                    SandBulb::Top => sample_y >= center_y && sample_y <= sample_mounded_fill,
                    SandBulb::Bottom => sample_y <= center_y && sample_y <= sample_mounded_fill,
                };
    ...
    if !fill_line_intersections.is_empty() {
        ...
        if fill_line_intersections.len() >= 2 {
            ...
            // Add curve points in the appropriate order based on bulb type
            match bulb {
                SandBulb::Bottom => {
                ...
                SandBulb::Top => {
                    // For top bulb, add curve points in reverse order
                    result_points.extend(curve_points.iter().rev());
                }
            }
        }
    }
